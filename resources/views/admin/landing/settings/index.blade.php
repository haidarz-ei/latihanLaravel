<x-app-layout>
    ...
    <div x-data="{ open:false, setting:null }">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="flex justify-between items-center mb-4">
                            ...
                            {{-- Tambah Button** --}}
                            <button @click="setting={key:'', value:'', type:'text', status:1}; open=true" 
                            ...
                            <tbody>
                                @foreach($settings as $setting)
                                    <tr class="border-t">
                                        ...
                                        <td class="px-3 py-2">
                                            <span class="{{ $setting->status ? 'bg-green-600 text-white' : 'bg-gray-400 text-gray-800' }}">
                                                {{ $setting->status ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td class="px-3 py-2 text-center">
                                            <button @click="open=true; setting={{ $setting->toJson() }}"
        ...
        <div x-show="open" x-transition class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl-full max-w-md shadow 2xl" @click.away="open=false">
                
                <h2 class="text-xl font-semibold mb-4"
                    x-text="setting.id ? 'Edit Setting' : 'Tambah Setting'">
                </h2>
                ...
                <form method="POST" :action="setting.id ? '/admin/landing/settings/' + setting.id : '{{ route('admin.landing.settings.store') }}'" 
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="key" class="w-full border rounded px-3 py-2" x-model="setting.key">
                    </div>
                    ...
                    <template x-if="setting.type !== 'image'">
                        <div class="mb-4">
                            ...
                            <textarea name="value" rows="5" class="w-full border rounded px-3 py-2" 
                                x-text="setting.value">
                            </textarea>
                        </div>
                    </template>
